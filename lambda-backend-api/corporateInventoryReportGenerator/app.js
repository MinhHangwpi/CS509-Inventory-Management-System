...
//    console.log(response.stock);
    ...
    temp_stock.reduce(function(res, value) {
      if (!res[value.SKU]) {
        res[value.SKU] = { "name":value.name,"description":value.description,"price":value.price,"SKU":value.SKU,"quantity":0, "item_val":0};
        grouped_stock.push(res[value.SKU])
      }
      res[value.SKU].quantity += value.quantity;
      res[value.SKU].item_val += value.quantity * value.price;
      return res;
    }, {});
